...
class MaterialRadioButton extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0, ?label:String = "", ?value:String = "", ?groupName:String = "default", ?selected:Bool = false,
			?onChange:String->Void = null)
	{
		...
		if (label.length > 0)
		{
			...
			labelText.setFormat(Paths.font('NotoSans-Medium.ttf'), 14, MD3Theme.onSurfaceVariant, LEFT);
	...
	override function update(elapsed:Float):Void
	{
		...
		var isOver = mousePos.x >= x && mousePos.x <= x + radioWidth && mousePos.y >= y && mousePos.y <= y + ICON_SIZE;
