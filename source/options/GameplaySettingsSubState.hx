...
class GameplaySettingsSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('Bad and Shit Break Combo',
			"If checked, hitting Bad or Shit notes will break your combo\nand count as Combo Breaks instead of just Misses.", 'badShitBreakCombo', BOOL);
		addOption(option);
		...
		var option:Option = new Option('Windows GDI Effects',
			'Allows Windows desktop GDI effects. Keep disabled unless you explicitly want mods to use them.', 'windowsGDIEffects', BOOL);
		addOption(option);
		option.onChange = onChangeWindowsGDIEffects;
		#end

		var option:Option = new Option('Pop Up Score',
			"If unchecked, hitting notes won't make \"sick\", \"good\".. and combo popups\n(Useful for low end " + Main.platform + ").", 'popUpRating', BOOL);
		addOption(option);
		...
		var option:Option = new Option('Use Flawless Rating', 'If checked, adds a tighter "Flawless!!" judgement above Sick.', 'useFlawlessRating', BOOL,
		...
		option.changeValue = 0.1;
		option.selectableCondition = function() return ClientPrefs.data.useFlawlessRating;
		addOption(option);
