...
class EditorPlayState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		var tipText:FlxText = new FlxText(10, FlxG.height - 24, 0,
			'Press ${(controls.mobileC) ? #if android 'BACK' #else 'X' #end : 'ESC'} to Go Back to Chart Editor', 16);
	...
	{
		if (#if android FlxG.android.justReleased.BACK #else touchPad.buttonP.justPressed #end
			|| controls.BACK
			|| FlxG.keys.justPressed.ESCAPE
			|| FlxG.keys.justPressed.F12)
		{
			endSong();
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (ClientPrefs.data.popUpRating)
		{
			...
			{
				var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(uiFolder + 'num' + Std.parseInt(separatedScore.charAt(i))
					+ PlayState.uiPostfix));
