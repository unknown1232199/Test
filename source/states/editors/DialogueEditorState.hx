...
class DialogueEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		var addLineText:FlxText = new FlxText(10, 10, FlxG.width - 20,
			'Press ${(controls.mobileC) ? 'A' : 'O'} to remove the current dialogue line, Press ${(controls.mobileC) ? 'X' : 'P'} to add another line after the current one.',
			8);
	...
	function addDialogueLineUI()
	{
		...
		#end
		var saveButton:PsychUIButton = new PsychUIButton(#if mobile 85, lineInputText.y + 25 #else loadButton.x + 120, loadButton.y #end, "Save Dialogue",
			function()
			{
				saveDialogue();
			});
		...
		if (character.animation.curAnim != null && character.jsonFile.animations != null)
		{
			animText.text = 'Animation: '
				+ character.jsonFile.animations[curAnim].anim
					...
					+ ') - Press ${(controls.mobileC) ? 'UP' : 'W'} or ${(controls.mobileC) ? 'DOWN' : 'S'} to scroll';
		}
		else
		{
	...
	function changeText(add:Int = 0)
	{
		...
		selectedText.text = 'Line: ('
			...
			+ ') - Press ${(controls.mobileC) ? 'LEFT' : 'A'} or ${(controls.mobileC) ? 'RIGHT' : 'D'} to scroll';
	}
