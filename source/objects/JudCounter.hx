...
class JudCounter extends FlxTypedGroup<FlxText>
{
	...
	function cacheLabels():Void
	{
		...
		var defaults = ['Flawlesss', 'Sicks', 'Goods', 'Bads', 'Shits', 'Misses', 'Combo', 'M. Combo'];
	...
	public function updateCounter(ratingsData:Array<Rating>, songMisses:Int, combo:Int, maxCombo:Int)
	{
		...
		var newValues = [
			Rating.getHits(ratingsData, 'flawless'),
			Rating.getHits(ratingsData, 'sick'),
			Rating.getHits(ratingsData, 'good'),
			Rating.getHits(ratingsData, 'bad'),
			Rating.getHits(ratingsData, 'shit'),
			songMisses,
			combo,
			maxCombo
		];
	...
	function updateSingleText(index:Int, value:Int):Void
	{
		...
		{
			case 0:
				flawlesssText.text = cachedLabels[0] + ': $value';
			case 1:
				sicksText.text = cachedLabels[1] + ': $value';
			case 2:
				goodsText.text = cachedLabels[2] + ': $value';
			case 3:
				badsText.text = cachedLabels[3] + ': $value';
			case 4:
				shitsText.text = cachedLabels[4] + ': $value';
			case 5:
				missesText.text = cachedLabels[5] + ': $value';
			case 6:
				comboText.text = cachedLabels[6] + ': $value';
			case 7:
				maxComboText.text = cachedLabels[7] + ': $value';
		}
	}
	...
	public function updateVisibility()
	{
		...
		flawlesssText.visible = shouldShow && ClientPrefs.data.useFlawlessRating;
		sicksText.visible = shouldShow;
		goodsText.visible = shouldShow;
		badsText.visible = shouldShow;
		shitsText.visible = shouldShow;
		missesText.visible = shouldShow;
		comboText.visible = shouldShow;
		maxComboText.visible = shouldShow;
		layoutTexts();
		lastVisibilityState = shouldShow;
	}

	function layoutTexts():Void
	{
		var index:Int = 0;
		if (ClientPrefs.data.useFlawlessRating)
			flawlesssText.setPosition(baseX, baseY + (spacing * index++));
		sicksText.setPosition(baseX, baseY + (spacing * index++));
		goodsText.setPosition(baseX, baseY + (spacing * index++));
		badsText.setPosition(baseX, baseY + (spacing * index++));
		shitsText.setPosition(baseX, baseY + (spacing * index++));
		missesText.setPosition(baseX, baseY + (spacing * index++));
		comboText.setPosition(baseX, baseY + (spacing * index++));
		maxComboText.setPosition(baseX, baseY + (spacing * index++));
	}
	...
	public function doBump(judgmentIndex:Int)
	{
		...
		{
			case 0:
				targetText = flawlesssText;
			case 1:
				targetText = sicksText;
			case 2:
				targetText = goodsText;
			case 3:
				targetText = badsText;
			case 4:
				targetText = shitsText;
			default:
