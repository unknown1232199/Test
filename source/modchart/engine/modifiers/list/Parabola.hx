...
class Parabola extends Modifier {
	...
	public function new(pf) {
		...
		amountIDs = [for (axis in AXES) findID('parabola' + axis)];
		offsetIDs = [for (axis in AXES) findID('parabola' + axis + 'Offset')];
	}
	...
	inline function applyAxis(curPos:Vector3, params:ModifierParameters, axisIdx:Int, realAxisIdx:Int):Void {
		...
		final shift = amount * t * t * ARROW_SIZE;
